...
- **Decision handshake** — `GET /api/v1/handshake` (called by every client SDK on session start; carries Switch + Config + Deploy + Catch + Analytics)
...
- **Catch** — `POST /api/catch/{events,transactions,vitals,profiles}`, `GET /api/v1/catch/issues`, `POST /api/v1/catch/symbols` (upload)
