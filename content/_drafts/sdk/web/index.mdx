...
```html
<script src="https://cdn.jsdelivr.net/npm/@sankofa/browser@0.1.1/dist/sankofa.min.js"></script>
<script>
  Sankofa.init({
    apiKey: "your-api-key",
...
The Web SDK ships Switch (feature flags) and Config (remote config)
as optional plugins alongside `@sankofa/browser`:

```bash
npm install @sankofa/switch @sankofa/config
```
...
The web Switch plugin also ships **per-call exposure tracking** —
deduplicated per session, batched every 5s to
`POST /api/switch/exposures`. See the full
[Switch + Config integration guide](/usage/switch-config-integration)
for identity stitching, bundled defaults, `onChange` subscriptions,
and exposure-tracker options.

To seed demo flags + config items for the bundled web example:
