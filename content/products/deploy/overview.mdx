---
title: Deploy
description: OTA (over-the-air) updates for mobile apps. React Native + Flutter, App-Store-compliant, gated by Switch decisions, auto-rolled-back by Catch error rates.
toc: true
---

**Deploy** is Sankofa's OTA (over-the-air) bundle delivery product. It ships code updates to React Native and Flutter apps without an App Store / Play Store review cycle, then watches the rollout and auto-rolls-back if Catch detects errors.

| Platform | SDK | Patch payload | API surface |
|---|---|---|---|
| **React Native** | `@sankofa/react-native` | JavaScript bundle | `SankofaDeploy` |
| **Flutter** | `sankofa_flutter` | Interpreted Dart code (runs in a VM) | `SankofaUpdater` |

Both ship under the same legal carve-outs — Apple PLA § 3.3.2 and Google Play DNA's interpreted-code-in-a-VM permission. See the [Flutter SDK overview](/sdks/flutter/overview#deploy-flutter-ota-updates) for the customer-facing API.
