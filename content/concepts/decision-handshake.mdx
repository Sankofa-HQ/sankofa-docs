...
description: One ETag-cached payload per session powers Switch, Config, Pulse triggers, and A/B Switch — together. Here's how the protocol works and why we built it this way.
toc: true
---
...
Sankofa's solution is the **decision handshake**: a single endpoint, called once per session, that returns the user's complete decision state across **every** product Sankofa runs — feature flags (Switch), remote config (Config), survey triggers (Pulse), and experiment assignments (A/B Switch). The SDK caches the response, evaluates everything locally for the rest of the session, and refreshes via ETag so unchanged state never re-hits the network.

This is the seam that makes "one binary, zero glue code" possible.

## The endpoint

<EndpointBadge method="POST" path="/api/v1/decisions" />
...
| `variant_assigned` | An A/B Switch experiment assigned the user to a variant via stable hashing. |
