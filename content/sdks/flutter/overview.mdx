...
<Steps>
  ...
  sankofa_flutter: ^0.2.10

# Sankofa-managed — do not edit by hand. Re-run \`sankofa init\` to regenerate.
dependency_overrides:
  # Temporary Android build fix — safe to remove once your engine tracks a newer
  # Flutter stable (see Troubleshooting).
...
# base_url: https://api.sankofa.dev  # optional — defaults to this if omitted`} />
    ...
    `init` and `login` are order-independent — whichever you run first, the other completes the setup. `engine_version` is stamped by the CLI (never hand-edit it), and `base_url` is optional — leave it out to use `https://api.sankofa.dev`, or set it to point at a self-hosted endpoint. The SDK reads everything else at runtime from the asset bundle.
  </Step>
  <Step title="sankofa_dynamic_interface.yaml — patchable surface">
    Written at the project root and passed to every build as `--dynamic-interface`. It declares which libraries a code-push patch may reach, so the release compiler retains the members a patch needs (`==`, `toString`, list/string helpers) instead of tree-shaking them out. `init` scaffolds it pointing at your app's entry library; extend the lists as your patchable surface grows.

    <CodeBlock lang="yaml" filename="sankofa_dynamic_interface.yaml" code={`callable:
  - library: 'package:your_app/main.dart'
extendable:
  - library: 'package:your_app/main.dart'
can-be-overridden:
  - library: 'package:your_app/main.dart'`} />
...
3. Add the `dependency_overrides:` (including the `shared_preferences_android: 2.4.23` pin) + `flutter.assets:` stanzas shown above to `pubspec.yaml`.
4. Create `sankofa.yaml` with your `app_id`, `api_key`, and the `engine_version` your build targets.
5. Create `sankofa_dynamic_interface.yaml` declaring your patchable libraries (snippet above).
6. Update `lib/main.dart` per the snippet above.
7. (Optional, for native crash bootstrapping) Extend `SankofaFlutterActivity` in `MainActivity.kt` and `SankofaFlutterAppDelegate` in `ios/Runner/AppDelegate.swift`.
