...
<Steps>
  <Step title="Add the SDK">
    `sankofa_flutter` already includes Deploy. No extra packages.
  </Step>
  <Step title="Create `sankofa.yaml`">
    At your project root, with your project's API key:

    <CodeBlock lang="yaml" filename="sankofa.yaml" code={`app_id: proj_xxxxxxxxxxxxx
api_key: sk_live_xxxxxxxxxxxxxxxx`} />

    Or run `sankofa init` and the CLI writes this for you.
  </Step>
  <Step title="Add `sankofa.yaml` to assets">
    <CodeBlock lang="yaml" filename="pubspec.yaml" code={`flutter:
  assets:
    - sankofa.yaml`} />
  </Step>
  <Step title="Apply staged patches in `main()`">
    <CodeBlock lang="dart" filename="lib/main.dart" code={`void main() async {
  WidgetsFlutterBinding.ensureInitialized();
  await SankofaUpdater.preFlight();   // applies any patch staged on the previous run
  runApp(const MyApp());
}`} />

    That's everything. The SDK reads `sankofa.yaml`, applies any patch that was downloaded on the previous run, and schedules a "patch healthy" confirmation 10s after the first frame paints. You do not import any helper packages.
  </Step>
</Steps>
