...
Sankofa Deploy ships bug fixes and UI tweaks to your already-released Flutter app without a new App Store / Play Store binary. Patches contain interpreted Dart code that runs in a VM inside the Sankofa-customized Flutter engine — explicitly permitted by Apple PLA § 3.3.2 and Google Play DNA's carve-out for interpreted code in a VM.
...
<Steps>
  <Step title="pubspec.yaml — SDK dependency">
    Adds the SDK and a managed `dependency_overrides:` stanza so the VM-binding helper resolves locally without exposing any GitHub URL in your project:
...
flutter:
  ...
  <Step title="sankofa.yaml — project keys">
    Written at the project root with `app_id` pre-filled from your login session:
...
api_key: <paste sk_live_* key from app.sankofa.dev → Settings → API Keys>`} />
    ...
    **You only need to paste the api_key.** The SDK reads everything else at runtime from the asset bundle.
  </Step>
  <Step title=".sankofa/ — local vendor + version pin">
    A sparse-clone of the `dynamic_modules` trampoline lands in `.sankofa/dynamic_modules/`, and `.sankofa/flutter-version` pins the bundled Sankofa Flutter SDK version. The `.sankofa/` dir is automatically added to your `.gitignore` — teammates re-run `sankofa init` on first checkout (~2 seconds).
  </Step>
...
Future<void> main() async {
  ...
  runApp(const MyApp());
    ...
    `registerLoader` plugs the VM's bytecode loader into the SDK once at boot; `preFlight` reads `sankofa.yaml`, applies any patch staged on the previous run, and schedules a "patch healthy" confirmation 10 seconds after the first frame paints.
  </Step>
...
After running, paste your `api_key` into `sankofa.yaml`, then `flutter pub get` and `flutter run`. Verify the setup with `sankofa doctor` — every check should be green.

<Callout tone="note" title="Why `dynamic_modules` is in dependency_overrides">
The VM binding (`loadModuleFromBytes`) imports `dart:_internal`, which pub.dev refuses to publish. The package is vendored locally so your `pubspec.yaml` contains **no GitHub URL** — only a managed override block. A future Sankofa release will move the binding into the bundled Flutter SDK as a `sdk: flutter` package and the override block will disappear entirely.
</Callout>
...
2. Clone `Sankofa-HQ/sankofa-dart-sdk` and copy `standalone/dynamic_modules/` to `<project>/.sankofa/dynamic_modules/`.
...
```bash
sankofa patch ios          # OR sankofa patch android
```

The CLI compiles your code change into bytecode, signs it with Ed25519 (when you've run `sankofa keys generate`), uploads to the storage bucket, and registers the release with your dashboard. Set the rollout (default 100%) and devices on their next `checkForUpdate()` call receive the patch.
