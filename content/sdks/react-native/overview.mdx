...
description: One package, all products — sankofa-react-native bridges to the native iOS and Android SDKs and ships analytics, errors, flags, config, surveys, replay, and JavaScript OTA Deploy.
toc: true
sdks: [react-native]
---

`sankofa-react-native` wraps the native [iOS](/sdks/ios/overview) and [Android](/sdks/android/overview) Sankofa SDKs and exposes a single TypeScript surface, including OTA Deploy — JavaScript bundle updates without a store review. (Flutter ships Deploy too, via the [Flutter SDK](/sdks/flutter/overview).)
...
<CodeBlock lang="tsx" filename="App.tsx" code={`import { NavigationContainer, useNavigationContainerRef } from "@react-navigation/native";
...
All four sources report through the same Catch pipeline. Sessions correlate by `distinct_id`.
