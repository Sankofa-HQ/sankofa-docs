...
<Steps>
  ...
  <Step title="Mobile — single SDK">
    Flutter, React Native, iOS, and Android each ship a single SDK that bundles every supported client product (Analytics, Catch, Switch, Config, Pulse, Replay). The **Flutter and React Native** SDKs also ship Deploy/OTA at the SDK level.
  </Step>
...
<CardGrid cols={3}>
  ...
  <Card href="/sdks/flutter/overview" eyebrow="Single SDK + Deploy" title="Flutter">
    `sankofa_flutter` on pub.dev. Six products + OTA Deploy (Dart code-push).
  </Card>
  <Card href="/sdks/react-native/overview" eyebrow="Single SDK + Deploy" title="React Native">
    `sankofa-react-native`. Six products + OTA Deploy (JS bundles).
  </Card>
...
- **Deploy / OTA ships in the Flutter and React Native SDKs.** Flutter code-push via `SankofaUpdater`; React Native JS bundles via `SankofaDeploy`. Native iOS / Android SDKs don't ship OTA — pure-native apps use Switch for feature gating + the platform's standard staged rollouts.
