...
- `--preview-artifact` — also build + upload an installable preview build (Android APK / iOS simulator app) so teammates can run this release with `sankofa preview --from-server`.
...
- **From server** (`--from-server`, or implied by `--label`/`--version`) — downloads a *published* release and installs it on an Android device/emulator or an iOS simulator, so a tester can run a specific build without your source. Requires the release to have been published with `--preview-artifact`.
