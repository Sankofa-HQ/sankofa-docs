...
| Command | Purpose |
|---|---|
| `create` | Scaffold a new Flutter app and run `init` in it — the fastest way to start from scratch. |
| `init` | Set up Sankofa in any project. Auto-detects platform; pass `--deploy`/`--catch`/`--flag`/`--config`/`--all` to skip the picker. |
| `account` | Show who you're logged in as, and your organizations / apps. |
...
sankofa login --endpoint https://api.self-hosted.example           # non-default server`} />
...
sankofa patch ios --release v1.2.0 --publish   # name the baseline (required in CI)
...
- `-t, --target <file>` — the app entry-point to rebuild + diff (default `lib/main.dart`). Pass your flavored main (e.g. `lib/main_prod.dart`); `--entry-file` is an alias. You just edit your real code — Sankofa diffs the rebuild.
...
Your patch-signing private key is stored locally — run `sankofa keys path` to locate it, then add it to your secrets manager. Losing the key means you can't ship signed patches against existing releases without a coordinated rotation.
...
sankofa flags list --include-archived                             # include archived
...
<Steps>
  ...
  <Step title="Global login">
    Your saved account login (from `sankofa login`).
  </Step>
</Steps>
