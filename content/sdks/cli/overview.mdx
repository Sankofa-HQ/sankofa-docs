...
Flutter patches ship interpreted code that runs in a VM inside the Sankofa-forked Flutter engine — App Store + Play Store compliant (Apple PLA § 3.3.2 + Google Play DNA carve-outs). React Native patches ship JavaScript bundles. Same `release` / `patch` / `deploy` syntax for both — the CLI detects the stack and runs the right pipeline.
...
sankofa patch ios                # push an iOS patch (no JIT entitlement required)`} />
...
<CodeBlock lang="bash" code={`# Flutter — Dart-only patch
sankofa patch android
sankofa patch ios                              # iOS — interpreted in a VM, no JIT entitlement
...
Flutter iOS options (Dart-only patches):

- `--target-binary-version <semver>` — match the host app's published version exactly (`pubspec.yaml.version`).
- `--label <label>` — patch label override (default `sankofa-ios-YYYYMMDDhhmmss`).
- `--engine-commit <sha>` — bake a specific Sankofa Flutter engine commit into the patch metadata.
...
Every Flutter patch is signed with the project's Ed25519 private key, and the server + on-device SDK both verify the signature before applying. Keys are project-scoped — generate one per project and back up the private key.
...
### `sankofa engine` — Sankofa Flutter engine binaries

Sankofa ships a customized Flutter engine that runs interpreted patches in a VM (App Store + Play Store compliant). The CLI caches per-ABI binaries locally and verifies SHA-256 against the registry on each download.
