...
For Flutter, `sankofa init --deploy` does the full setup automatically (see the [Flutter SDK page](/sdks/flutter/overview#deploy--flutter-ota-updates)): adds the Sankofa Flutter SDK, creates `sankofa.yaml` with `app_id` pre-filled, wires `lib/main.dart`, and patches the Android + iOS native files. You only need to paste your api_key into `sankofa.yaml` afterward.
...
- `--engine-version <version>` — override the bundled Sankofa Flutter runtime version (rare).
