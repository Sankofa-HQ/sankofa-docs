...
<ParameterTable
  params={[
    ...
    { name: "signing_pubkey", type: "string?", description: "The base64 Ed25519 public key patches are signed with. sankofa keys generate prints it; you paste it here (or pass it to Sankofa.initialize(deploySigningPubkey:)). The SDK verifies every patch against it before applying." },
  ]}
/>

<Callout tone="warning" title="Runtime config vs. the API contract">
The app's runtime SDK is configured with `apiKey` + `endpoint` and authenticates to the ingestion API with the `x-api-key` header — never `appKey` or `serverUrl`. In `sankofa.yaml` those two values live under `api_key` and (optionally) `base_url`; the CLI maps them for you. Don't hand-edit `engine_version` — the CLI stamps it. `signing_pubkey` is the one key you add yourself, from the `sankofa keys generate` output.
</Callout>
