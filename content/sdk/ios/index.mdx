...
> `SankofaConfig` (above) is the init-options struct passed to
> `Sankofa.shared.initialize(...)`. The **remote-config** module is
> called `SankofaRemoteConfig` — different responsibility, different
> type.
...
`SankofaSwitch` and `SankofaRemoteConfig` are singletons that
self-register with the Module Registry on first access, so there's no
separate init call beyond seeding bundled defaults:
...
The iOS SDK forwards `anon_id` on handshake after `Sankofa.shared.identify(...)`
fires, so experiment queries stitch pre- and post-login rows for the
same device. See the full
[Switch + Config integration guide](/usage/switch-config-integration)
for bundled defaults, `onChange` tokens, and the `SankofaRemoteConfig`
↔ `SankofaConfig` naming rationale.
...
Then open [`example/sankofa_example_ios`](https://github.com/Sankofa-HQ/sankofa/tree/main/example/sankofa_example_ios)
in Xcode and run — every surface in the **Lab** tab (plus the variant
CTA on the Tracking tab + maintenance banner) repaints live from the
dashboard.
